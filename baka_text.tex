...
\subsubsection{Predikce jako způsob zjednodušení zápisu}
...
\subsubsection{Predikce jako inspirace}\td{isnt that too… poetic, if I may? (ten nadpis myslím) xD}
...
S ohledem na velké množství metod zadávání textu, z nichž mnohé obsahují technologie prediktivního textu pro ušetření počtu nutných stisků kláves, je nutné mít možnost objektivním způsobem testovat jednotlivé metody, porovnávat je a hodnotit jejich skutečné možnosti. Pro tyto účely slouží metoda výpočtu KSPC (zkratka pro keystrokes per character). Výpočet čísla, které lze pak použít pro srovnávání, se provádí pomocí vzorce, který bere v potaz  udává průměrný počet stisků kláves nutných k zadání jednoho znaku danou metodou\footnote{přestože název obsahuje stisky kláves, veličina se používá i pro měření účinnosti zadávání textu např. pomocí stylusu na dotykovém zařízení}.
